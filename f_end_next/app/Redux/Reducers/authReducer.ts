...
export const actions = {
  getAuthTokenAC: (token: string) =>
    ({ type: "auth/LOGIN", payload: token } as const),
  getAuthMeAC: (me: string) => ({ type: "auth/ME", payload: me } as const),
  FetchRegisterErrorAC: (error: string) =>
    ({ type: "auth/FETCH_REGISTER_ERROR", payload: error } as const),
  FetchRegisterAC: (response: string) =>
    ({ type: "auth/FETCH_REGISTER", payload: response } as const),
  FetchLogErrorAC: (error: string) =>
    ({ type: "auth/FETCH_LOGIN_ERROR", payload: error } as const),
  FetchLoginAC: (response: string) =>
    ({ type: "auth/FETCH_LOGIN", payload: response } as const),
  IsFetchingLoginAC: (isFetching: boolean) =>
    ({ type: "auth/IS_FETCHING_LOGIN", payload: isFetching } as const),
  IsFetchingRegisterAC: (isFetching: boolean) =>
    ({ type: "auth/IS_FETCHING_REGISTER", payload: isFetching } as const),
};
...
export const register = (
  ...
  return async (dispatch: DispatchType) => {
    ...
    await authAPI
      .Register(username, phoneNumber, password, password2)
      .then((res) => dispatch(actions.FetchRegisterAC(res.data)))
      .catch((err) =>
        dispatch(actions.FetchRegisterErrorAC(err.response.data))
      );
